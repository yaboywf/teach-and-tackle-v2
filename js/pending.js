...
async function getUser(adminNum) {
    try {
        const resp = await axios.get(`https://s5y8kqe8x9.execute-api.us-east-1.amazonaws.com/api/account/account-information?id=${encodeURIComponent(adminNum.toUpperCase())}`);
        return resp.data || {};
    } catch (err) {
        console.error(err);
        ...
        return `
            <div class="request" id="${request.pair_id}">
                <div class="student_info">
                    <img src="${otherUser.image_key ? `https://teach-and-tackle-images.s3.us-east-1.amazonaws.com/${otherUser.image_key}` : "../images/favicon.webp"}" alt="Profile Picture">
                    <div>

...
.then(resp => {
    document.querySelector(".user").style.setProperty("--before-background", resp.data.image_key ? `url(https://teach-and-tackle-images.s3.us-east-1.amazonaws.com/${resp.data.image_key}) center/cover no-repeat` : "url(/images/favicon.webp) center/cover no-repeat");
})
.catch(err => {
    console.error(err);
